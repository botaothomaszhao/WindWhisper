package moe.tachyon.windwhisper.logger
...
import moe.tachyon.windwhisper.logger.WindWhisperLogger.getLevelName
import moe.tachyon.windwhisper.logger.WindWhisperLogger.parseLoggerName
import moe.tachyon.windwhisper.logger.WindWhisperLogger.safe
import moe.tachyon.windwhisper.utils.LineOutputStream
import moe.tachyon.windwhisper.utils.LinePrintStream
import moe.tachyon.windwhisper.workDir
import java.io.*
...
object WindWhisperLogger
{
    ...
    fun getLogger(): LoggerUtil = getCallerClass()?.let(WindWhisperLogger::getLogger) ?: globalLogger
    ...
    fun String.parseLoggerName(): String =
        if (this.startsWith("moe.tachyon.windwhisper.")) "~." + this.substring("moe.tachyon.windwhisper.".length)
        else this
}
        ...
        this.formatter = object: Formatter()
        {
            override fun format(record: LogRecord): String
            {
                ...
                val head = if (loggerConfig.showLoggerName) String.format(
                    "%s[%s]%s[%s]%s[%s]%s",
                    PURPLE.bright(),
                    WindWhisperLogger.loggerDateFormat.format(record.millis),
                    CYAN.bright(),
